$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    "ajax": "../sayfalar/grid_data.php?job=get_companies",
    "columns": [
      { "data": "rank" },
  ...
  $(document).on('submit', '#form_company.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          '../sayfalar/grid_data.php?job=add_company',
        cache:        false,
        data:         form_data,
        dataType:     'json',
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          '../sayfalar/grid_data.php?job=get_company',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('submit', '#form_company.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_company').serialize();
      var request   = $.ajax({
        url:          '../sayfalar/grid_data.php?job=edit_company&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', '.function_delete a', function(e){
    ...
    if (confirm("Are you sure you want to delete '" + company_name + "'?")){
      ...
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          '../sayfalar/grid_data.php?job=delete_company&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
