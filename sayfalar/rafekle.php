...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
  ...
  if(isset($_REQUEST['op']) && strtoupper($_REQUEST['op'])==strtoupper("getData") ){
     $id = $_REQUEST["id"];
     $sql = "SELECT * FROM RAFLAR WHERE ID='".$id."' ORDER BY ID asc";
  ...
  exit;  
	 ...
	 $id = $_REQUEST["ID"];
     $sql = "DELETE FROM RAFLAR WHERE ID=".$id;
	     if ($pdo->query($sql) == false) {
      $result  = 'error';
      $message = 'query error';
    } else {
  ...
  echo $json_data;
	...
	$id 					= $_REQUEST["ID"];
	$SERI 					= $_REQUEST["SERI"];
	$KAPASITE 				= $_REQUEST["KAPASITE"];
	$OLUSTURAN 				= $_SESSION['username'];
	if ($id!=null)
		{
			$sql = "UPDATE RAFLAR set SERI='$SERI',KAPASITE='$KAPASITE'  WHERE ID= ".$id;	
		}
	else 
	 {	
		$sql = "INSERT INTO RAFLAR (SERI,KAPASITE, OLUSTURAN) VALUES ('$SERI','$KAPASITE','$OLUSTURAN')";
	...
	if ($pdo->query($sql) == false) {
		...
		$basari= " Raf başarı ile eklendii."; 
		$result  = 'success';
		$message = 'query success';
    }	
  $pdo=null;
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Hoşgeldiniz
        <small>Raf İşlemleri</small>
      </h1>
    </section>
    <section class="content">
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Raf Yönetim Ekranı</h3>
						...
						<div class="col-lg-12">
										...
										<div class="row" style="margin-top:10px">	
											<div class="col-sm-3"	>
												<div class="form-group has-success">
														...
														<input type="text" class="form-control" name="SERI" id="SERI" required="required" />
												</div>
											</div>
										</div>		
										<div class="row" style="margin-top:10px">	
											<div class="col-sm-3"	>
												<div class="form-group has-success">
														<h4> <span class="label label-primary">Kapasite</span></h4>
													
														<input type="text" class="form-control" name="KAPASITE" id="KAPASITE" required="required" />
												</div>
											</div>
										</div>											
								</div>
							...
							<div class="col-xs-12">
									...
									$(document).ready(function(){
										$('#table_RAFLAR tbody').on('click', 'tr', function () {
											
											var table = $('#table_RAFLAR').DataTable();
											...
											document.getElementById("SERI").value 			= response.data.seri;
											document.getElementById("KAPASITE").value 			= response.data.kapasite;
											});
										});
									});	
								</script>
									...
									<table class="datatable" id="table_RAFLAR">
										<thead>
											<tr>
												<?php 
													//gridBaslikOlustur(array('S.No','Firma Ünvanı','Temsilcisi','Telefonu'));
													gridBaslikOlustur(array('S.No','RAF ADI','KAPASITE','OLUSTURAN'));
												?>
											</tr>
										</thead>
										<tbody>
									...
									<script type="text/javascript">
										...
										var tabloadi= "table_RAFLAR";
										var sayfaadi="../kontroller/rafgrid.php";
										var parameter =  [
											   { "data": "row_num"},
											   { "data": "SERI" },
											   { "data": "KAPASITE" },
							...
							<script>
								$(document).ready(function () {
								  ...
								  $("#btnSil").click(function (event) {
									var mesaj = "";
									var tablex = $('#table_RAFLAR').DataTable();
										if($("#ID").val()=="" )
										{
